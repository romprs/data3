...
которого настраиваются в конфигурационном файле. Пока не истекло настроенное
время, окно нельзя закрыть, свернуть или переключиться на другое приложение.
После истечения времени в окне появляется кнопка закрытия.

## Требования

- РЕД ОС (или другой RPM-дистрибутив на базе systemd), графическая сессия X11.
- Пакеты: `python3-gobject3`, `gtk3`, `systemd`.
