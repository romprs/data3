...
def _display_name() -> str:
    """Имя пользователя для приветствия из GECOS (полное имя из учётной
    записи). GECOS в корпоративных доменах заполняется по русскому
    порядку «Фамилия Имя Отчество», поэтому для приветствия берутся
    второе и третье слово (имя и отчество), без фамилии — если
    отчества нет, используется только имя, а если и его нет —
    системный логин."""
