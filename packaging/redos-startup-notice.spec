...
- Исправлена регрессия при обновлении пакета: старый config.ini (от версий
  ...
  tasks_file/phrases_file/background_dir вовсе, из-за чего после
  обновления пакета переставали показываться примеры фото и фраз дня;
  теперь отсутствующий ключ берёт файл/каталог, который ставит сам пакет
  (пустое значение по-прежнему осознанно отключает блок)
- Приветствие теперь берёт из GECOS учётной записи имя, а не фамилию
  (для формата "Фамилия Имя Отчество" использовалось первое слово)
